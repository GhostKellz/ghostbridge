...
pub export fn zcrypto_ed25519_keypair(public_key: [*]u8, private_key: [*]u8) callconv(.C) c_int {
    ...
    const keypair = shroud.crypto.sign.ed25519.KeyPair.create(null) catch return -1;
...
pub export fn zcrypto_ed25519_sign(
    ...
    const keypair = shroud.crypto.sign.ed25519.KeyPair{
...
pub export fn zcrypto_ed25519_verify(
    ...
    shroud.crypto.sign.ed25519.verify(sig, message[0..message_len], pub_key) catch return -1;
...
pub export fn zcrypto_secp256k1_keypair(public_key: [*]u8, private_key: [*]u8) callconv(.C) c_int {
    ...
    const secret_key = shroud.crypto.sign.secp256k1.SecretKey.random();
    const pub_key = shroud.crypto.sign.secp256k1.PublicKey.fromSecretKey(secret_key);
...
pub export fn zcrypto_secp256k1_sign(
    ...
    const secret_key = shroud.crypto.sign.secp256k1.SecretKey{
        .data = private_key[0..SECP256K1_PRIVATE_KEY_SIZE].*,
    };
    
    // Sign the message hash (should be 32 bytes)
    const sig = shroud.crypto.sign.secp256k1.sign(message_hash[0..32].*, secret_key) catch return -1;
...
pub export fn zcrypto_blake3_hash(input: [*]const u8, input_len: usize, output: [*]u8) callconv(.C) c_int {
    ...
    var hasher = shroud.crypto.hash.blake3.init(.{});
...
pub export fn zcrypto_sha256_hash(input: [*]const u8, input_len: usize, output: [*]u8) callconv(.C) c_int {
    ...
    var hasher = shroud.crypto.hash.sha256.init(.{});
